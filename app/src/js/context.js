...
import { closeModal, disableButton, messageHTMLtoText, openModal } from './display';
...
function positionMenu(t,e){
  ...
  const windowWidth=window.innerWidth;const windowHeight=window.innerHeight;windowWidth-clickCoordsX<menuWidth?e.style.left=windowWidth-menuWidth+"px":e.style.left=clickCoordsX+"px",windowHeight-clickCoordsY<menuHeight?e.style.top=windowHeight-menuHeight+"px":e.style.top=clickCoordsY+"px"
}
...
function setContextFolderItems(item, element) {
  ...
  contextItemList.eq(3).get(0).onclick = () => {
    ...
    $(`#deleteFolderConfirm`).get(0).onclick = () => {
      closeModal();
      deletePlaylistFolder(folderID, folderName);
    }
  }
