  ...
  return new Promise((resolve, reject) => {
    ...
    if (['jpg', 'jpeg', 'png'].includes(file.name.split('.').pop().toLowerCase())) {
      uploadTask.then(() => { // Resolve with the resized path.
        resolve(`https://firebasestorage.googleapis.com/v0/b/parallel-by-wop.appspot.com/o/attachmentsPreview%2F${dmKEYify(user.uid, uID)}%2F${user.uid}%2F${timeOrID}-resized.${file.name.split(".").pop()}?alt=media`);
      })
    }
    else {
      uploadTask.then(() => {
        resolve(`https://firebasestorage.googleapis.com/v0/b/parallel-by-wop.appspot.com/o/attachments%2F${dmKEYify(user.uid, uID)}%2F${user.uid}%2F${timeOrID}.${file.name.split(".").pop()}?alt=media`);
      })
    }
  })
}
