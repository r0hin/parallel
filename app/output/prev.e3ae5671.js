...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/prev.js":[function(require,module,exports) {
"use strict";
...
const db = (0, _firestore.getFirestore)(); // Get ID from URL Params
...
async function getPlaylist(playlistUID, playlistID) {
  try {
    ...
    if (playlistDoc.data().sharing == 'friends') {
      throw new Error("This playlist is private");
    }

    const coverURL = playlistDoc.data().imageURL || `https://firebasestorage.googleapis.com/v0/b/parallel-by-wop.appspot.com/o/app%2FdefaultAlbum.png?alt=media&token=589fb415-f487-4f4c-b6b5-5c150eb1ceec`; // const doc = playlistDoc;
    ...
    $(`#content`).html(`
      ...
      <div>
    `);
    twemoji.parse($(`#playlistDescription`).get(0));
    $(`#${playlistUID}${playlistID}playlistDetailsLine`).html(`By ${cloneText} <span class="playlistDetailsSeparator"><i class="bx bxs-circle"></i></span> Updated ${timeago.format(modifiedDate)} <span class="playlistDetailsSeparator"><i class="bx bxs-circle"></i></span> ${playlistDoc.data().tracks ? playlistDoc.data().tracks.length : 0} track${playlistDoc.data().tracks && playlistDoc.data().tracks.length !== 1 ? "" : "s"} ${lengthString}${lockText}`); // Tracks Rendering (Feb 9, 2022): 250 batches, paginatated.
    ...
    console.log(error);
    snac('Permission Denied', 'This playlist is not public.', 'error');
...
} catch (error) {} // Insert el at index via $().insertIndex()
