...
async function getPlaylist(playlistUID, playlistID) {
  try {
    ...
    $(`#${playlistUID}${playlistID}playlistDetailsLine`).html(`By ${cloneText} <span class="playlistDetailsSeparator"><i class="bx bxs-circle"></i></span> Updated ${timeago.format(modifiedDate)} <span class="playlistDetailsSeparator"><i class="bx bxs-circle"></i></span> ${playlistDoc.data().tracks ? playlistDoc.data().tracks.length : 0 } track${playlistDoc.data().tracks && playlistDoc.data().tracks.length !== 1 ? "" : "s"} ${lengthString}${lockText}`);
