  ...
  if (filePath.includes("pfp/") || filePath.includes("covers/") || filePath.includes('groups/') || filePath.includes('playlists/')) {
    ...
    if (filePath.includes('groups/')) {
      await admin.firestore().collection('users').doc(filePath.split('/')[1]).collection('servers').doc(filePath.split('/')[2]).update({
        url: `https://firebasestorage.googleapis.com/v0/b/parallel-by-wop.appspot.com/o/${JPEGFilePath.replace(/\//g, '%2F')}?alt=media&ts=${new Date().getTime()}`
      })
    }

    if (filePath.includes('users/')) {
      await admin.firestore().collection('users').doc(filePath.split('/')[1]).update({
        url: `https://firebasestorage.googleapis.com/v0/b/parallel-by-wop.appspot.com/o/users%2F/${filePath.split('/')[1]}/profile.png?alt=media&ts=${new Date().getTime()}`
      })
    }
    ...
    if (filePath.includes('playlists/')) {
      await admin.firestore().collection('users').doc(filePath.split('/')[1]).collection('playlists').doc(filePath.split('/')[2]).update({
        imageURL: `https://firebasestorage.googleapis.com/v0/b/parallel-by-wop.appspot.com/o/${JPEGFilePath.replace(/\//g, '%2F')}?alt=media&ts=${new Date().getTime()}`,
...
exports.resizeImages = functions.runWith({
  ...
  const bucket = admin.storage().bucket('parallel-by-wop.appspot.com');
...
exports.manageDeletions = functions.storage.bucket('parallel-by-wop.appspot.com').object().onDelete(async (object) => {
  // Catch attachments
  const filePath = object.name;
